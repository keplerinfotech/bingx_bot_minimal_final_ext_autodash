...
class Router:
    ...
    def __init__(self, venues: list, latency_models: dict | None = None):
        ...
        self.latency_models = latency_models or {
            name: LatencyModel() for name in self.venues.keys()
        }
    ...
    def estimate_flow_rate(self, venue: Venue, ts, lookback_ms=60 * 60 * 1000):
        ...
        return {
            "buy_ms": s.get("buy", 0.0) / dt_ms,
            "sell_ms": s.get("sell", 0.0) / dt_ms,
        }

    def expected_fill_probability(
        self, venue: Venue, ts, side: str, price: float, qty: float, horizon_ms=60_000
    ):
        """
        Heuristic model:
    ...
    def rank_venues(self, ts, side, price, qty, horizon_ms=60_000):
        ...
        for v in self.venues.values():
            p = self.expected_fill_probability(
                v, ts, side, price, qty, horizon_ms=horizon_ms
            )
