...
def find_sweeps(
    ...
    vol_z = (vol - vol.rolling(100, min_periods=20).mean()) / (
        vol.rolling(100, min_periods=20).std(ddof=0) + 1e-12
    )
    ...
    for i in range(len(df)):
        ...
        up_ok = (
            up_break
            and up_wick
            and (vol_z.iloc[i] >= vol_burst_z)
            and (up_reject if require_reject else True)
        )
        ...
        dn_ok = (
            ...
            and dn_wick
            and (vol_z.iloc[i] >= vol_burst_z)
            and (dn_reject if require_reject else True)
        )
            ...
            out.append(
                {
                    ...
                    "context": {
                        "type": "up-sweep",
                        "roll_max_hi": float(roll_max_hi.iloc[i]),
            ...
            out.append(
                {
                    ...
                    "context": {
                        "type": "down-sweep",
                        "roll_min_lo": float(roll_min_lo.iloc[i]),
                        "vol_z": float(vol_z.iloc[i]),
                    },
                }
            )
    ...
    return (
        pd.DataFrame(out).set_index("timestamp")
        if out
        else pd.DataFrame(
            columns=["timestamp", "direction", "entry_price", "invalidation", "context"]
        ).set_index("timestamp")
    )
