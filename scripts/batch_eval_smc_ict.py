...
def run_backtest(tp, sl, params):
    ...
    try:
        ...
        if output.startswith("{") and output.endswith("}"):
            ...
            result.update(
                {"trades": 0, "expectancy": 0.0, "profit_factor": 1.0, "max_dd": 0.0}
            )
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        future_to_params = {
            executor.submit(run_backtest, tp, sl, p): (tp, sl, p)
            for tp in TPS
            for sl in SLS
        ...
        for idx, future in enumerate(as_completed(future_to_params), 1):
            ...
            results.append(res)
            logging.info(
                f"[PROGRESS] {idx}/{len(future_to_params)} done | tp={tp} sl={sl} dur=~ err={res['trades']==0}"
            )
    ...
    top_risk_adj = sorted(
        results,
        key=lambda x: (x["expectancy"] / max(x["max_dd"], 0.0001)),
        reverse=True,
    )[:3]
    ...
    for i, r in enumerate(top_exp, 1):
        print(
            f"[{i}] exp={r['expectancy']:.6f} PF={r['profit_factor']:.3f} trades={r['trades']} TP={r['tp']} SL={r['sl']} params={r}"
        )

    print("\n--- Top by Risk-Adjusted (expectancy / max_dd) ---")
    for i, r in enumerate(top_risk_adj, 1):
        print(
            f"[{i}] ratio={(r['expectancy']/max(r['max_dd'],0.0001)):.2f} equity=?? max_dd={r['max_dd']:.3f} exp={r['expectancy']:.6f} PF={r['profit_factor']:.3f} TP={r['tp']} SL={r['sl']} params={r}"
        )
