...
def discover_copies(root: str = ".") -> Dict[str, List[str]]:
    """
    Return mapping canonical_path -> [duplicate_paths...]
    Canonical is <dir>/<base><ext>, duplicates match '<base> copy( N)?.ext' in same dir.
    """
...
def process_group(
    ...
    if not canon_exists:
        ...
        def _dupe_num(p: str) -> int:
            m = COPY_RE.match(os.path.basename(p))
        ...
        sorted_dupes = sorted(existing_dupes, key=_dupe_num)
        ...
        # Remove promoted from duplicates list
        existing_dupes = [d for d in existing_dupes if d != promote]

    # Now canonical must exist
